<template>
  <div class="relative m-4 mt-3 sm:mb-2 pointer-events-auto">
    ...
    <textarea
      ...
      rows="3" ref="textareaDom" v-model="store.messageContent"
      :placeholder="placeholder || $t('ChatSendBar.placeholder')" @keydown="(event) => enterInput(event)"
      @compositionstart="composing = true" @compositionend="composing = false"></textarea>
    <ChatSendButton class="absolute bottom-2 sm:bottom-2 right-3" :is-talking="store.talking" @click="sendMessage" />
  </div>
</template>
